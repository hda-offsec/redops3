...
class HeaderFuzzer:
    ...
    def audit_headers(self, port, protocol='http', logger=None):
        ...
        try:
            baseline_resp = http_client.get(url, options=getattr(self, "options", None), timeout=5, allow_redirects=False)
        ...
        try:
            r_host = http_client.get(url, options=getattr(self, "options", None), headers={"Host": test_host}, timeout=5, allow_redirects=False)
        ...
        for header, values in self.fuzz_headers.items():
            for val in values:
                try:
                    r_bypass = http_client.get(url, options=getattr(self, "options", None), headers={header: val}, timeout=5, allow_redirects=False)
        ...
        for payload in self.injection_payloads:
            try:
                ...
                r_ua = http_client.get(url, options=getattr(self, "options", None), headers={"User-Agent": payload}, timeout=5)
