...
class AccessControlScanner:
    ...
    def scan_403_bypass(self, port, protocol='http', logger=None):
        ...
        try:
            # Baseline
            r_base = requests.get(target_url, timeout=3, allow_redirects=False)
            if r_base.status_code in [401, 403]:
                ...
                for h, v in headers_bypass.items():
                    try:
                        ...
                        r_test = requests.get(target_url, headers={h: v}, timeout=3, allow_redirects=False)
                ...
                for v in variations:
                    ...
                    try:
                        r_var = requests.get(url_var, timeout=3, allow_redirects=False)
